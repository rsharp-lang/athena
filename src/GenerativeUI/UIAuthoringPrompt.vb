...
Public Module UIAuthoringPrompt
    ...
    Public Function Build(host As JavascriptInterop, Optional extraRules As String = Nothing) As String
        ...
        Call sb.AppendLine("## 常见错误（务必避免）")
        Call sb.AppendLine("1. 不要使用 document.getElementById('xxx') 之后再直接调用方法——一旦 id 写错就会拿到 null 并抛出异常，")
        Call sb.AppendLine("   表现为「按钮点了完全没有反应」。请改用 `GenUI.el('xxx')`：它取不到元素的时候会自动创建一个空容器，")
        Call sb.AppendLine("   并把这个情况报告给宿主，不会让整个功能失效。")
        Call sb.AppendLine("2. 不要在宿主对象上调用任何方法。与宿主的通信一律通过 `GenUI.call`，")
        Call sb.AppendLine("   尤其不要使用 `host.invoke`，这个名字在 COM 之上是不可用的。")
        Call sb.AppendLine("3. 不要引用任何外部资源：没有 CDN、没有外链字体、没有外链图片，所有 CSS 与 JS 都必须内联。")
        Call sb.AppendLine("4. 不要使用 ES module（type=""module""）、import / export、document.write。")
        Call sb.AppendLine("5. 每一个 `await` 都必须包在 try { } catch (e) { } 之中，并且把错误用")
        Call sb.AppendLine("   `GenUI.status(e.message, 'error')` 显示到界面上；否则异常会变成未处理的 Promise rejection，")
        Call sb.AppendLine("   用户只会看到「点了没反应」，而宿主完全不知情。")
        Call sb.AppendLine("6. 结果容器请先在 HTML 之中写成空标签占位（例如 `<div id=""result-images""></div>`），")
        Call sb.AppendLine("   再由脚本填充内容；不要依赖脚本去创建这些 id 对应的元素。")
        Call sb.AppendLine("7. 参数控件必须覆盖参数清单之中的**每一个**参数，不能遗漏，")
        Call sb.AppendLine("   否则宿主收集到的参数会不完整，脚本会拿不到该参数的值。")
        Call sb.AppendLine()
